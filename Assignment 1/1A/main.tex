...
Then, by using the code in appendix \ref{appendix:code}, we obtain :
...
Therefore we can conclude that $q^*(\mu) = \mathcal{N}(\mu|\mu', \lambda'^{-1})$ with :
\begin{equation}
    \mu'     = \frac{\lambda_0\mu_0 + \sum_{n=1}^{N}x_n}{\lambda_0 + N}
\end{equation}
\begin{equation}
...
Therefore we can conclude that $q^*(\tau) = \text{Gam}(\tau|a', b')$ with :
\begin{equation}
    a' = a_0 + \frac{N}{2}
\end{equation}
\begin{equation}
    \begin{split}
        b' & = b_0 + \frac{1}{2}\mathbb{E}_\mu\left[\sum_{n=1}^{N}(x_n - \mu)^2 + \lambda_0(\mu - \mu_0)^2\right]                                                                                                     \\
        b' & = b_0 + \frac{1}{2}\left(\sum_{n=1}^{N}x_n^2 + N\mathbb{E}_\mu[\mu^2] - 2\mathbb{E}_\mu[\mu]\sum_{n=1}^{N}x_n + \lambda_0\left(\mathbb{E}_\mu[\mu^2] + \mu_0^2 - 2\mu_0\mathbb{E}_\mu[\mu]\right)\right) \\
    \end{split}
\end{equation}
...
We have also some formulas to detail :
\begin{equation}
    \begin{split}
        \mathbb{E}_\mu[\mu]   & = \mu'                        \\
        \mathbb{E}_\mu[\mu^2] & = \frac{1}{\lambda'} + \mu'^2 \\
        \mathbb{E}_\tau[\tau] & = \frac{a'}{b'}
    \end{split}
\end{equation}

The rest of the answer is in the code in appendix \ref{appendix:code}.
