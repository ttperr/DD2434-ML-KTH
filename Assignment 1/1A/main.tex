...
\begin{equation}
    \begin{split}
         ...
         & = \sum_{k=1}^{K}\mathbb{E}_{q(\beta_{k})}[\log p(\beta_{k})] + \sum_{d=1}^{D}\mathbb{E}_{q(\theta_d)}[\log p(\theta_d)] + \sum_{d=1}^{D}\sum_{n=1}^{N}\mathbb{E}_{q(z_{dn})}[\log p(z_{dn}|\theta_d) + \log p(w_{dn}|\beta_{z_{dn}})] \\
