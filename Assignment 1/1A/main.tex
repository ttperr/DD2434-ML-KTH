...
And therefore:
\begin{equation}
    \frac{1}{\mathbb{E}[\tau]} = \frac{1}{N-1}(\overline{x^2} - \overline{x}^2) = \frac{1}{N-1}\sum_{n=1}^{N}(x_n - \overline{x})^2
\end{equation}

The implementation is in the code in appendix \ref{appendix:code.3.13}.
...
\subsection{Question 3.16}\label{appendix:code.3.16}
%\lstinputlisting[label = {alg:3.16}]{py_files/3.16.py}
